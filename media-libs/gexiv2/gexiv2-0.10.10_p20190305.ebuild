...
if [[ ${PV} == 9999 ]]; then
	...
	SRC_URI="mirror://gnome/sources/${PN}/$(ver_cut 1-2)/${PN}-$(ver_cut 1-3).tar.xz"
...
PATCHES=(
	...
	# is rather awkward
	"${FILESDIR}"/${PN}-$(ver_cut 1-3)-meson-fixup.patch
	"${FILESDIR}"/${PN}-$(ver_cut 1-3)-vala-fixup.patch
	"${FILESDIR}"/${PN}-$(ver_cut 1-3)-internal-comment.patch
	"${FILESDIR}"/${PN}-$(ver_cut 1-3)-repeatable.patch
	"${FILESDIR}"/${PN}-$(ver_cut 1-3)-zero-size.patch
	"${FILESDIR}"/${PN}-$(ver_cut 1-3)-langalt.patch
)
